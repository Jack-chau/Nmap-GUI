...
### scan_network
    ...
    def clean_database( self ) :
        delete_id = self.del_id_box.get()
        cur = self.conn.cursor()
        cur.execute( f"DELETE FROM scanning_history WHERE id = { delete_id }" )
        self.conn.commit()
        self.clean_textbox( )
        self.textbox.insert('end', f"\n id: {delete_id} data in database has been deleted\n" )
